...
export interface IAverageBuyPayload {
  symbol: string;
  strategyName: string;
  exchangeName: string;
}
...
interface IParams {
  payload: {
    getContext: () => IAverageBuyPayload;
    reloadSubject: TSubject<void>;
  };
}

export const useAverageBuyModal = ({ payload }: IParams) => {
  const { pickData, render } = useWizardModal({
    history,
    animation: "none",
    title: t("Average Position"),
    AfterTitle: ({ onClose }) => (
...
export default useAverageBuyModal;
