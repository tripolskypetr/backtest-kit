 ...
 * База имеет микроскопический дрейф вверх (+1e-4% в минуту). Hit =
 * достижимость: всплеск +1% покрывает симметричную цель стопа 1%
 * (lock=0), автор проходит дефолт-бан фильтра.
 *
 ...
 * (вечный холд = кап горизонта идеи). Стоп 1% в монотонном мире не
 * срабатывает (low не опускается ниже входа), трейлинг отключён
 * сентинелем TT=100, поэтому выход — только по времени:
