...
export class LogMockService {
    ...
    public getOne = async (id: string): Promise<ILogEntry | null> => {
        this.loggerService.log("logMockService getOne", { id });
        const { data, error } = await fetchApi(
            `/api/v1/mock/log_one/${id}`,
            {
                method: "POST",
                body: JSON.stringify({
                    clientId: CC_CLIENT_ID,
    ...
    public findByFilter = async <T extends object = Record<string, string>>(
        ...
        this.loggerService.log("logMockService findByFilter", { filterData, limit, offset });
        const { data, error } = await fetchApi(
            "/api/v1/mock/log_filter",
            {
                method: "POST",
                body: JSON.stringify({
                    clientId: CC_CLIENT_ID,
