...
const PRICE_REACTION_PROMPT = str.newline(
  ...
  ' - Ты торгуешь в направлении сентимента (bearish → SHORT, bullish → LONG).',
  ' - Стоит ли прямо сейчас открывать позицию? Смотри на последние свечи периода: есть ли ещё пространство для движения в направлении сентимента, или цена уже достигла дна/потолка и разворачивается?',
  ' - **enter**: тренд в направлении сентимента активен или цена только начала движение — есть потенциал.',
  ' - **wait**: цена уже достигла экстремума и разворачивается в обратную сторону — входить поздно, риск поймать отскок.',
  '',
  '**Требуемый результат:**',
