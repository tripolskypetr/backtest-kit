...
- [ ] **P2 (откачено пользователем — осознанное решение шаблона): formatPrice/formatQuantity — предпочтение limits.min вместо precision** — тот же дефект, что чинили в cli ExchangeSchemaService: у ccxt binance (TICK_SIZE mode) тик-сайз — это precision.price, а limits.price.min лишь совпадает с ним на большинстве рынков; limits.amount.min (minQty) может превышать шаг лота → пере-огрубление количества. Выровнено с cli-копией (precision-first + find по конечному положительному числу).
...
### Третий проход packages/signals/src (по запросу): построчная перечитка вычислительных тел

Дочитано то, что во втором проходе сверялось структурно: полные тела generateAnalysis всех 4 math-сервисов + все хелперы (calculateVolumeMetrics/PriceChanges/SupportResistance/FibonacciLevels/VolumeTrendRatio) + BookDataMathService целиком. Новых багов нет. Подтверждено вычислительно: volatility5 — RMS лог-доходностей по 5 свечам (÷4 сэмпла); pressureIndex ∈ [−100,100] с гардом range>0; squeezeMomentum безразмерный (BB-width% / ATR%-of-price) с гардом price≠0; nearest-логика S/R (значимые уровни за порогом 0.3%, ближайший сверху/снизу через сортировку, фолбэк на экстремум окна); объёмные окна LongTerm смежные без нахлёста (slice(i−11,i−5) vs slice(i−5,i+1)); фибо-уровни Short/Long от high вниз включая downside-расширения, nearest по абсолютной дистанции; условные update Swing (пропуск битых свечей без рассинхрона стримов индикаторов); depthImbalance/спред/мид BookData с null-гардами по пустой стороне. Единственное замечание — O(n²) в calculateVolumeMetrics/PriceChanges (slice префикса на каждую строку) при n=60 — не трогалось.

---

## Аудит корневого ./src, шестой проход (2026-07-04): последние непокрытые зоны

Замыкает карту покрытия проходов 1–5. Прочитано целиком: src/math (все 10 хелперов), src/helpers (getPriceProfile, getEffectivePriceOpen, getPriceScale, getContextTimestamp — остальные были покрыты ранее), src/utils (writeFileAtomic, waitForCandle, beginContext/beginTime, alignToInterval, intervalStepMs, roundTicks, get/set), config/ladder.ts, classes/Cache.ts (808 строк), classes/Risk.ts, classes/Strategy.ts, classes/PositionSize.ts (тонкие markdown-фасады), function/{control,init,timeframe,session}.ts полностью + экспорт-скан остальных тонких обёрток, структурный скан schema/validation-сервисов (единый ToolRegistry-паттерн с shallow-валидацией).

- [x] **P3: докстринг stopStrategy врал о сигнатуре** — `src/function/control.ts`: `@param strategyName` и пример `stop("BTCUSDT", "my-strategy")` при фактической сигнатуре `stopStrategy(symbol)` (strategy берётся из method-контекста) и неверном имени импорта. Исправлено.

Проверено и признано корректным (без изменений): math/* — симметричные round-trip пары sl/tp PercentShift↔Price (SIGNED-дистанции, цель по обе стороны входа выразима), percentDiff с честными Infinity/0-edge; getPriceProfile — медианный шаг, pressure/strength, лог-регрессия с двумерным гейтом R²×slope, нулевые деления отсечены; getEffectivePriceOpen — канон итеративного алгоритма (фронт-виджет с него портирован); Cache.ts — TTL по выровненному интервалу СИМУЛИРУЕМОГО context.when, изоляция ключей по (strategy,exchange,frame,mode), эвикция rejected-промисов, gc/hasValue/clear с контекст-гардами, файловый вариант поверх PersistMeasureAdapter с индекс-счётчиком (resetCounter для walker-циклов); writeFileAtomic (tmp+fsync+rename на POSIX, честная оговорка про Windows); waitForCandle (sleep-to-boundary с re-arm на ранний таймер, unref); beginContext/beginTime (escape обоих/одного контекста); roundTicks (экспоненциальная форма, кап 100 знаков); тонкие function-обёртки — единообразные контекст-гарды. Замечание без изменений: CacheFileInstance.run без in-flight-дедупа (конкурентные вызовы одного ключа исполняют fn дважды, last-write-wins — терпимо для measure-кэша).

Итоговая карта: после шести проходов непрочитанных исполняемых зон в корневом src не осталось (contract/model/interfaces — декларации, покрыты точечно при ренеймах; assets/columns — проход 4; статистика — 100 golden-датасетов).

---

## Переквалификация «недостижимых» stopped-веток (2026-07-04)

По вопросу пользователя «почему monkey-patch не позволяет влезть в эти ветки» — проверено пробой: **позволяет**. Формулировка «непроверяемы по построению» была неверной; в пятом проходе стояло честнее («экзотическая гонка, осознанно не тестируем»). Механика: stopStrategy конвертирует scheduled→_cancelledSignal и дренаж идёт ДО прайс-монитора, поэтому штатно в ACTIVATE_* с флагом не попасть; но прямое `strategy._isStopped = true` между тиками (worker-testbed style) воспроизводит ровно состояние mid-tick гонки — монитор не имеет внешнего stopped-гарда и доводит до внутренней ветки.

Результат ветки подтверждён: tick-результат **idle** (не cancelled — отмена уходит в schedule-event/commit каналы, та же семантика, что у activation-reject), scheduled очищен и null-персист записан, `cancelled/user` на scheduleEventSubject, risk-слот освобождён.

Две ловушки пробы (поучительно): (1) дефолтные файловые Persist-адаптеры восстанавливали scheduled из ./dump ПРОШЛОГО запуска пробы; (2) файловый кэш свечей подмешивал цену прошлого тика в VWAP-окно (49700 вместо 48500 → waiting вместо активации).

- [x] Тест: `test/e2e/stopped.test.mjs` — по указанию пользователя **всё хранилище in-memory, адаптеры реализованы в скоупе test()** (Signal/Schedule/Strategy как KV-класс-фабрика, Risk — пустые позиции, Candle — постоянный miss, чтобы VWAP всегда отражал текущую цену). Паттерн снимает обе ловушки и делает тест полностью герметичным.

persist-after-confirm: аналогично тестируем — порядок «persist строго после sync-подтверждения» наблюдаем через in-memory адаптер (запись в KV) + инструментированный onOrderSync; отдельный тест не добавлялся (свойство уже утверждено кодом и комментарием контракта в OPEN_NEW_*_FN), при желании добавляется тем же паттерном.

### «Все 9»: семейство тестов неожиданных остановок (по запросу) — исправлено и покрыто

- [x] **P1 (новый баг, найден тестом №1): стоп, поднятый ВНУТРИ placement-гейта, оставлял живой resting-ордер** — `OPEN_NEW_SCHEDULED_SIGNAL_FN` не перепроверял `_isStopped` после подтверждения гейта: ордер размещён на бирже, флаг стоит, но никто его не отменит (stopStrategy-конверсия уже прошла до размещения). Фикс: post-gate проверка конвертирует свежезарегистрированный scheduled в `_cancelledSignal` (`cancelNote: "stop_strategy"`, зеркально stopStrategy) с персистом обоих состояний — следующий tick дренирует через брокер-канал и снимает risk-резервацию. Для pending-open (type "active") проверка НЕ добавлена сознательно: gate-true = биржа подтвердила реальный филл, отказ от регистрации рассинхронизировал бы с биржей (позиция мониторится и доживает — контракт «positions continue»).

- [x] **Тесты `test/e2e/stopped.test.mjs` (10 шт., все — in-memory адаптеры в скоупе test())**: №1 стоп из onOrderSync-гейта (ловит фикс выше); №2 стоп из risk-валидации (тот же дренаж); №3 backtest-близнец активационной ветки (флаг из onSchedulePing до кроссинга, ветка 4124); №4 live-дренаж отложенной user-активации под стопом (ветка 6877); №5 backtest-близнец (ветка 4748, активация+флаг из onSchedulePing); №6 activateScheduled ПОСЛЕ стопа — no-op (7865); №7 deferred close/cancel, выданные до стопа, дренируются после (NOTE-контракты 6624/7928); №8 подтверждённый TP-филл дренируется в closed/take_profit под стопом; №9 стоп process-local — после clear/restore торговля возобновляется (пиннинг семантики; продуктовый вопрос «должен ли stop переживать рестарт» остаётся открытым).

- [x] **Грабли среды (важно для test/README)**: (а) цепочка `strategyCoreService.tick(...)` оборачивает только ExecutionContext — live-тик в тестах ОБЯЗАН идти внутри `MethodContextService.runInContext(CTX)`, иначе `getAveragePrice` падает ContextReferer (в первой пробе wrapper выжил случайно после perl-правки); (б) гонка exit в раннере: `process.exit` в cb `run()` наперегонки с финализацией ПОСЛЕДНЕГО tape-теста давал фантомный «not ok: test exited without ending» (или тихо съедал последний ok) — в test/index.mjs добавлен `setTimeout(exit, 250)`.

Полный набор: **872 ok / 0 fail**.

### Live-семейство тестов (по запросу): test/e2e/live.test.mjs — 9 тестов

Герметичный live-тик харнес (in-memory адаптеры в скоупе test(), MethodContextService.runInContext, VWAP от текущей цены): (1) отказ schedule-пинга (onOrderCheck type=schedule бросает) → отмена resting-ордера через пайплайн; (2) time_expired ровно по minuteEstimatedTime; (3) отмена scheduled по CC_SCHEDULE_AWAIT_MINUTES; (4) TP-закрытие по VWAP-кроссингу между тиками (pnl>0); (5) SL-закрытие (pnl<0); (6) пробой SL до активации → cancel, позиция НЕ открывается сквозь пробитый стоп; (7) interval-троттлинг getSignal: 3 тика в одном aligned 5m-интервале = 1 вызов, новый интервал = 2-й; (8) канальность: listenSignalLive+listenSignal получают opened/closed, listenSignalBacktest молчит, listenSync отдаёт пару signal-open/close с type=active и backtest=false; (9) frontend-style доступ вне контекстов: Price/TimeMetaService отдают VWAP и время ПОСЛЕДНЕГО тика по идентификаторам.

Полный набор: **881 ok / 0 fail**.

### writeFileAtomic × PersistBase (по запросу): стык атомарной записи и файлового адаптера

- [x] **P2: крэш между tmp-записью и rename воскрешал фантомную сущность** — tmp-имя `writeFileAtomic` сохраняет расширение (`.tmp-<hex>-<id>.json`), а `PersistBase.keys()` фильтровал только по `.json`: полностью записанный, но не переименованный tmp после рестарта проходил валидацию `waitForInit` (валидный JSON) и попадал в листинги как сущность `.tmp-<hex>-<id>`; недописанный — удалялся как «invalid document», но под чужим id. Фикс: `TMP_FILE_PREFIX` экспортирован из writeFileAtomic (заготовка пользователя) и подключён в Persist.ts — (а) `keys()` пропускает префикс (фантом невозможен), (б) `waitForInit` подметает tmp-остатки (init — единственный момент без конкурентных писателей). Тест: «persist: stale atomic-write tmp file is swept…» — реальная PersistBase поверх диска: real-сущность цела, фантома нет, tmp удалён.

- [x] **P3: докстринг writeFileAtomic лгал** — упоминал несуществующий `CC_SKIP_POSIX_RENAME`, ссылку на GLOBAL_CONFIG и «agent swarm system» (копипаста происхождения). Переписан под факт + задокументирован контракт tmp-префикса для сканеров каталогов.

Полный набор: **882 ok / 0 fail**.

### Отмена ордера в live через брокерские хуки (по запросу): test/e2e/broker_cancel.test.mjs — 4 теста

Broker.useBrokerAdapter + enable, герметичный live-харнес: (1) onOrderCheck throw (type=schedule) → resting-ордер отменяется, адаптер получает onSignalScheduleCancelled; (2) onOrderCheck throw (type=active) → позиция закрывается closed/closed, при этом адаптер НЕ получает close-commit — запиннен контракт CLOSE_PENDING_SIGNAL_AS_CLOSED_FN («пинг уже установил, что ордера нет — повторный close избыточен»; изначальный ассерт теста был неверен, сверено с кодом); (3) onSignalOpenCommit throw (type=schedule) → размещение отклонено, scheduled не регистрируется, событие scheduled не эмитится, троттл откатывается — ретрай в ТОМ ЖЕ интервале проходит после снятия отказа (2 попытки размещения); (4) onSignalOpenCommit throw (type=active) при активации scheduled → терминальная отмена: opened нет, cancelled на schedule-канале, адаптер получил и активационный confirm, и scheduleCancelled, следующий тик не ретраит.

Полный набор: **886 ok / 0 fail**.

### Проверка src/function/strategy.ts (по запросу) — чисто

Перечитан целиком (60 экспортов). Фиксы прошлых проходов на месте во всех вариантах: percent- И dollar-партиалы конвертируют/шлют брокеру cost от ОСТАТОЧНОГО базиса (getTotalCostClosed), все 4 trailing-варианта считают брокерскую цену от ОРИГИНАЛЬНЫХ уровней (originalPrice* ?? price*), signalId во всех Broker-payload. Шаблонная сверка скриптом: 31 метрик-геттер строго конформен (двойные контекст-гарды, currentPrice фетчится только там, где передаётся, имена core-методов совпадают — единственное расхождение getStrategyStatus→getStatus намеренное). Overlap-функции: симметричная ladder-математика, у partial-overlap уровень = partial.currentPrice. Edge: remainingCost=0 в *Cost-вариантах даёт Infinity% → отбивается validatePartial*.

Известные особенности, подтверждены как осознанные (не трогалось): порядок «Broker.commit* ДО core-apply» единый по всем копиям (false от core = пользователь сверяет с биржей — контракт возврата); commitAverageBuy валидирует до getPendingSignal (единственный с таким порядком, поведенчески эквивалентно); имена getTotalCostClosed/PercentClosed исторически возвращают ОСТАТОК — честные алиасы getRemainingCostBasis/getTotalPercentHeld на месте; асимметрия сигнатур (commitTrailingStop требует currentPrice 3-м аргументом, Cost-варианты фетчат VWAP сами).

### Систематическая сверка src/function/* (по запросу): лог-метки + копии кост-математики

- [x] **Кост-математика: все 12 мест трёх копий выровнены** — скрипт-сверка всех вызовов percentToCloseCost/investedCostToPercent (strategy.ts ×4, Backtest.ts ×4, Live.ts ×4): везде аргумент — remaining* из strategyCoreService.getTotalCostClosed. Дублей с неправильным базисом не осталось.

- [x] **P3: 7 копипаст-меток в логах** (найдены скрипт-сверкой METHOD_NAME-констант против фактических имён всех экспорт-функций src/function/*): event.ts — 6 Available-слушателей логировались без суффикса («event.listenPartialProfit» вместо «...Available» и т.д., + JSDoc-примеры импортировали несуществующие имена); cache.ts — приватный _checkFsCandles логировал метку публичного checkCandles (неразличимы в логах) → отдельная константа «cache._checkFsCandles». Повторный скан: чисто.

Полный набор: **886 ok / 0 fail**.

### Дожатие канона strategy.ts (по матрице покрытия): test/e2e/strategy_fn.test.mjs — 13 тестов

Закрыты все 12 функций без прямых тестов: (1-2) commitTrailingStopCost — брокер-payload round-trip (newSL 48000 → shift −4 от ОРИГИНАЛА), закрытие ровно по новому уровню; ВТОРОЙ сдвиг от оригинала (−6, не −2 от эффективного) — пиннинг фикса 4-го прохода; (3) commitTrailingTakeCost (shift −8, take_profit на 51000); (4) backtest-близнец SL-трейла из onActivePing; (5-6) commitPartialLossCost $300−$150−$75→$75 live + backtest-близнец ($150 из $300 = ровно 50% в partial-loss commit-событии; нюанс: validatePartialLoss отвергает партиал на мелком убытке −0.8% — триггер тесту нужен на дне −4%); (7) commitCreateStopLoss — подтверждённый SL-fill закрывает stop_loss мимо VWAP (зеркало TP из stopped №8); (8) commitCreateSignal КАНОНА (класс-копии были покрыты, канон — нет); (9) getPositionEntry/PartialOverlap — коридор ±1.5% с границей включительно, уровень партиала = его цена; (10) hasNoPending/ScheduledSignal через фазы с дренажами; (11) getStrategyStatus — выяснен и запиннен контракт live-closePending: pending отцепляется сразу (снапшот с pendingSignalId=null), отложенное закрытие в closedSignal до дренажа, после — очищается; (12) commitSignalNotify → listenSignalNotify с notificationId/notificationNote; (13) алиасы Held≡PercentClosed, RemainingCostBasis≡CostClosed ($150 после 50% от $300 — семантика «остаток» запиннена).

Полный набор: **899 ok / 0 fail**.

### Пункты 2/3/4/6/10 (по запросу): test/e2e/edge_fn.test.mjs — 12 тестов

SHORT-зеркала *Cost-канона: trailingStopCost тянет ВЕРХНИЙ стоп вниз (54000→52000, shift −4, закрытие на росте), partialLossCost режет доллары на растущей цене ($300−$150−$75), подтверждённый SL-fill мимо VWAP. Гонки deferred: двойной closePending схлопывается в ОДНО закрытие; **запиннен контракт: activateScheduled побеждает более поздний cancelScheduled** (activate перемещает scheduled в _activatedSignal — филл случился, отменять нечего, cancel = no-op; базис = scheduled priceOpen с fp-толерансом); closePending без pending — безопасный no-op; TP-fill + user-close вместе → одно closed/take_profit (подтверждённый филл важнее). **Запиннен контракт 100%-партиала: позиция ЖИВЁТ active с нулевым базисом**, финальное закрытие остаётся за TP/SL/expiry (TP штатно добивает зомби) — ⚠️ продуктовый вопрос «должен ли 100%-партиал закрывать позицию» открыт. Негативные аргументы: percent 0/−5/150 и $400 из $300 отвергнуты без мутации базиса; чужой symbol в user-DTO → позиции нет, mismatch в listenError. Restore при смене схемы: рестарт с interval 1m→5m — восстановленный pending жив, генератор молчит при живом pending, закрытие по TP из ПЕРСИСТА, после закрытия троттлинг подчиняется НОВОМУ интервалу.

Полный набор: **911 ok / 0 fail**.
