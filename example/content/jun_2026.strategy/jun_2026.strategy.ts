 ...
 *
 * Точка: Sharpe 2.21, +20.5% за июнь, 8 сделок, 0 стопов, dd 3.45%.
 *  - HARD_STOP 7%: чистая страховка — ни одна консенсусная сделка
 *    июня до него не доставала (H=7 эквивалентен отключённому стопу
 *    по факту, но ограничивает катастрофу). Sharpe-максимум сетки
 *    (+26% при N=1 БЕЗ стопа) отвергнут: он требует пересиживать
 *    внутрисделочные провалы глубже -7% — неограниченный риск.
 *  - TRAILING_TAKE 3%: по Sharpe лучше 4% (отдаём от пика меньше);
 *    узкий трейлинг (<1.5%) закрывается об шум минуток.
 ...
 *    однонаправленных автора. Именно консенсус-фильтр удерживает
 *    внутрисделочную просадку < 7% — одиночные идеи (N=1) ныряют
 *    глубже и живут только без стопа.
 *
 * Честная оговорка по бейзлайну: чистый time-exit без стопа и
 * трейлинга даёт +19.2% — выходы добавляют ~1-3 п.п. Эдж в самих
