...
class ReportStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      `*Trend Strength: computed FROM CLOSING PRICES OF CLOSED SIGNALS (one (closeTimestamp, currentPrice) point per closed trade — no candles, no order book, no tick stream). Ordinary-least-squares slope of \`log(close) ~ days\` across that series; expressed in **%/day** (small-slope approximation, log-return ≈ percent-return). NOT an ATR-normalised slope. NOT ADX. NOT a rolling-window indicator computed at signal entry — static fit over the entire stored history at report time. Null when N < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Trend Confidence (R²): computed FROM CLOSING PRICES OF CLOSED SIGNALS. Coefficient of determination of the same log-price regression. On **log(close)**, NOT on absolute close. Range [0, 1]; high R² = log-price moves linearly with time (clean trend); low R² = noisy regardless of slope sign. Null when N < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Trend (classification): computed FROM CLOSING PRICES OF CLOSED SIGNALS. "sideways" when R² < 0.30 (regression too weak to call any direction). Otherwise "neutral" when |slope| < 0.25 × medianStepSize (slope detectable but smaller than the typical daily step). Otherwise "bullish" if slope > 0, "bearish" if slope < 0. Two-axis gate (R² × slope/step); never a single-axis if-on-magic-constant.*`,
      `*Buyer Pressure / Seller Pressure: computed FROM CLOSING PRICES OF CLOSED SIGNALS. FREQUENCY-based: buyerPressure = (count of i where close[i] > close[i−1]) / (count of decisive moves). sellerPressure = (count of down-moves) / (count of decisive moves). Flats (close[i] == close[i−1]) excluded from both numerator and denominator. buyerPressure + sellerPressure = 1 by construction. UNITS: dimensionless fraction in [0, 1]. NOT order-flow buy/sell volume — backtest data has no order book; "buyer/seller" labels the SIGN of the close-to-close return.*`,
      `*Buyer Strength / Seller Strength: computed FROM CLOSING PRICES OF CLOSED SIGNALS. MAGNITUDE-based: buyerStrength = Σ |close[i] − close[i−1]| / close[i−1] over up-moves, divided by the same sum over ALL decisive moves. sellerStrength = same over down-moves. Σstrength = 1 by construction. UNITS: dimensionless fraction in [0, 1]. Pressure (count) and Strength (Σ|Δ|) use the SAME close-to-close return series — only count vs magnitude differs. Divergence (e.g. buyerPressure 0.70 with buyerStrength 0.45) means "many small up-moves, fewer but larger down-moves" — a regime asymmetry the frequency alone hides.*`,
      `*Pressure Imbalance: computed FROM CLOSING PRICES OF CLOSED SIGNALS. DIFFERENCE, not ratio: buyerStrength − sellerStrength. UNITS: dimensionless, range [−1, +1]. Sign = direction of magnitude bias; magnitude = how lopsided. Equivalent to (2 × buyerStrength − 1).*`,
      `*Median Step Size: computed FROM CLOSING PRICES OF CLOSED SIGNALS. Median over i ∈ [1..N−1] of |close[i] − close[i−1]| / close[i−1] — the PERCENT step between consecutive trade closes. "Step" = consecutive closes of CLOSED TRADES (not ticks, not bars of any timeframe — the report has no candle access). UNITS: percent, normalised by price → directly comparable between BTC and a low-priced symbol. Median (not mean) → robust to one whale trade. NOT classical candle-based volatility (ATR / σ of returns); it measures the step distribution AT THE RATE TRADES CLOSE.*`,
