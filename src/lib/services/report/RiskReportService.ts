...
export class RiskReportService {
  ...
  private tickRejection = async (data: RiskEvent) => {
    ...
    await ReportWriter.writeData("risk", {
      ...
      worstStalePercentage: data.currentSignal?.worstStale?.pnlPercentage,
      worstStaleCost: data.currentSignal?.worstStale?.pnlCost,
      worstStaleEntries: data.currentSignal?.worstStale?.pnlEntries,
      worstStalePriceOpen: data.currentSignal?.worstStale?.priceOpen,
      worstStalePriceClose: data.currentSignal?.worstStale?.priceClose,
      worstStalePeakPercentage: data.currentSignal?.worstStale?.peakPnlPercentage,
      worstStaleGivebackPercentage: data.currentSignal?.worstStale ? data.currentSignal.worstStale.peakPnlPercentage - data.currentSignal.worstStale.pnlPercentage : undefined,
      worstStaleMinutes: data.currentSignal?.worstStale ? Math.floor((data.currentSignal.worstStale.timestamp - data.currentSignal.worstStale.peakTimestamp) / 60000) : undefined,
      worstStaleBreakevenPrice: data.currentSignal?.worstStale?.breakevenPrice,
      worstStaleBreakevenTimestamp: data.currentSignal?.worstStale?.breakevenTimestamp,
    }, {
      symbol: data.symbol,
      strategyName: data.strategyName,
      exchangeName: data.exchangeName,
