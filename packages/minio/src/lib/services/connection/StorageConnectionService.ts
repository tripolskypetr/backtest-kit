...
const REDIS_KEY = "storage-items-connection";
...
export class StorageConnectionService extends BaseMap(REDIS_KEY, -1) {
  readonly loggerService = inject<LoggerService>(TYPES.loggerService);

  public register = async (objectName: string): Promise<void> => {
    this.loggerService.log("storageConnectionService register", { objectName });
  ...
  public listNewest = async (limit: number, prefix = ""): Promise<string[]> => {
    this.loggerService.log("storageConnectionService listNewest", { limit, prefix });
    const redis = await getRedis();
