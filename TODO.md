...
- [x] **Контекстно-независимые методы ClientStrategy (все кроме tick/backtest)** (834 ok / 0 fail)
  Продолжение пользовательского фикса `WAIT_FOR_DISPOSE_FN` (dispose падал с `ScopeContextError` при вызове из `clear()` вне контекстов): выметены чтения `execution.context`/`method.context` из кода, достижимого методами вне tick/backtest — `WAIT_FOR_INIT_FN` (23 чтения → статические params; timestamps restore-коллбеков → `Date.now()`, путь live-only где when == wall clock), `setPendingSignal` (symbol/backtest → params), `setScheduledSignal` (полностью контекст-фри). Замены эквивалентны: params — тот же triple, по которому инстанс мемоизирован. Единственное оставшееся чтение вне tick/backtest — ленивый `execution.context.when` в setPendingSignal для метки onWrite (в backtest это время свечи, wall clock не замена; вызывается только из пайплайнов) — задокументировано в коде. Deferred-команды/getters/partial/trailing/breakeven контекстов не читали и раньше. tick/backtest не тронуты (76+35 чтений — их законный контекст). Coverage-тест crash-recovery переведён на ГОЛЫЙ `clear()` без обёрток — фиксирует контекст-фри dispose. В test/README.md зафиксировано, кто какой контекст создаёт: `strategyCoreService.tick` сам оборачивает в ExecutionContext из аргументов — снаружи нужен только MethodContext (ниже core-слоя — оба).
