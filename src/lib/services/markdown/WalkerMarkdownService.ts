...
class ReportStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      `*Sortino Ratio (best strategy summary): the best strategy's avgPnl / downsideDeviation (risk-free rate = 0). downsideDeviation = √( Σ min(0, pnlPercentage)² / N ), summed over the strategy's closed signals and divided by the total signal count (canonical Sortino 1991, MAR = 0). UNITS: dimensionless. N/A when the best strategy has fewer than 10 closed signals, when it has no losing trades, or when downsideDeviation is below a float-artifact threshold (~1e-9). Rule of thumb: below 1.0 poor, 1.0–2.0 acceptable, above 2.0 strong.*`,
      `*Calmar Ratio (best strategy summary): the best strategy's expectedYearlyReturns / equityMaxDrawdown, clamped to ±1000. expectedYearlyReturns is the geometric annualisation of the strategy's compounded equity curve; equityMaxDrawdown is the mark-to-market max drawdown of that same curve (each trade's worst intra-trade excursion is applied as a trough before booking the realised close, so deep round-trip dips count rather than only close-to-close drops). UNITS: dimensionless. N/A when expectedYearlyReturns is N/A (e.g. too few signals, calendar span too short, or trade frequency too clustered to extrapolate) or when equityMaxDrawdown ≤ 0. Rule of thumb: below 0.5 poor, above 1.0 strong.*`,
      `*Recovery Factor (best strategy summary): the best strategy's (equityFinal − 1) × 100 / equityMaxDrawdown, clamped to ±1000. The numerator is the strategy's compounded total return (the equity curve's end value minus its start, in percent) — NOT the arithmetic sum of per-trade PNLs; mixing those units would inflate the ratio on long winning streaks. The denominator is the same mark-to-market max drawdown described under Calmar Ratio. UNITS: dimensionless. N/A when the best strategy has fewer than 10 closed signals, the strategy blew its account (equity ≤ 0), or equityMaxDrawdown ≤ 0. Rule of thumb: below 1.0 means profit doesn't cover drawdown, above 3.0 is good.*`,
