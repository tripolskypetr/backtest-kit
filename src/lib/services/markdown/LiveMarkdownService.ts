...
class ReportStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      `*Trend Strength: computed FROM CLOSING PRICES OF CLOSED SIGNALS (one (event.timestamp, event.currentPrice) point per closed event; Live ingests sub-minute events but the price profile uses ONLY closed-event prices for parity with Backtest — no candles, no order book, no tick stream). Ordinary-least-squares slope of \`log(close) ~ days\` across that series, in **%/day** (small-slope approximation). NOT ATR-normalised. NOT ADX. NOT a rolling-window indicator at signal entry — static fit at report time. Null when N < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Trend Confidence (R²): computed FROM CLOSING PRICES OF CLOSED SIGNALS. Coefficient of determination of the same log-price regression. On **log(close)**, NOT on absolute close. Range [0, 1]; high R² = clean trend; low R² = noisy regardless of slope sign. Null when N < ${MIN_SIGNALS_FOR_RATIOS}.*`,
      `*Trend (classification): computed FROM CLOSING PRICES OF CLOSED SIGNALS. "sideways" when R² < 0.30. Otherwise "neutral" when |slope| < 0.25 × medianStepSize. Otherwise "bullish" if slope > 0, "bearish" if slope < 0. Two-axis gate (R² × slope/step); never a single-axis if-on-magic-constant.*`,
      `*Buyer Pressure / Seller Pressure: computed FROM CLOSING PRICES OF CLOSED SIGNALS. FREQUENCY-based: buyerPressure = (count of i where close[i] > close[i−1]) / (count of decisive moves). sellerPressure = (count of down-moves) / (count of decisive moves). Flats excluded. buyerPressure + sellerPressure = 1. UNITS: fraction in [0, 1]. NOT order-flow aggressor volume — Live's ingest does not separate buy/sell side; "buyer/seller" labels the SIGN of the close-to-close return.*`,
      `*Buyer Strength / Seller Strength: computed FROM CLOSING PRICES OF CLOSED SIGNALS. MAGNITUDE-based: buyerStrength = Σ |close[i] − close[i−1]| / close[i−1] over up-moves, divided by the same sum over ALL decisive moves. sellerStrength = same over down-moves. Σstrength = 1. UNITS: fraction in [0, 1]. Pressure (count) and Strength (Σ|Δ|) use the SAME return series; divergence reveals regime asymmetry (frequent shallow up + rare deep down, etc).*`,
      `*Pressure Imbalance: computed FROM CLOSING PRICES OF CLOSED SIGNALS. DIFFERENCE, not ratio: buyerStrength − sellerStrength. UNITS: dimensionless, range [−1, +1]. Sign = direction of magnitude bias.*`,
      `*Median Step Size: computed FROM CLOSING PRICES OF CLOSED SIGNALS. Median over i ∈ [1..N−1] of |close[i] − close[i−1]| / close[i−1] — the PERCENT step between consecutive closed events. "Step" = consecutive closed-event prices, NOT ticks / NOT bars. UNITS: percent (normalised by price), directly comparable across symbols at any price level. Median → robust to outliers. NOT classical candle volatility — measures the step distribution at the rate trades close.*`,
