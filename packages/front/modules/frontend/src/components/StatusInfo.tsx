...
const toMarkdown = (data: StatusInfoModel): string => {
    ...
    for (const s of symbols) {
        lines.push(
            ...
            `**Trend:** ${s.trend ?? "—"}  `,
            `**Trend Strength:** ${s.trendStrength === null ? "—" : `${s.trendStrength > 0 ? "+" : ""}${s.trendStrength.toFixed(3)}%/day`}  `,
            `**Trend Confidence (R²):** ${s.trendConfidence === null ? "—" : s.trendConfidence.toFixed(3)}  `,
            `**Buyer Pressure:** ${s.buyerPressure === null ? "—" : `${(s.buyerPressure * 100).toFixed(1)}%`}  `,
            `**Seller Pressure:** ${s.sellerPressure === null ? "—" : `${(s.sellerPressure * 100).toFixed(1)}%`}  `,
            `**Buyer Strength:** ${s.buyerStrength === null ? "—" : `${(s.buyerStrength * 100).toFixed(1)}%`}  `,
            `**Seller Strength:** ${s.sellerStrength === null ? "—" : `${(s.sellerStrength * 100).toFixed(1)}%`}  `,
            `**Pressure Imbalance:** ${s.pressureImbalance === null ? "—" : `${s.pressureImbalance > 0 ? "+" : ""}${s.pressureImbalance.toFixed(3)}`}  `,
            `**Median Step Size:** ${fmt(s.medianStepSize)}  `,
