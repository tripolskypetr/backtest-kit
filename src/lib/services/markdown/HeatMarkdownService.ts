...
class HeatmapStorage {
  ...
  public async getReport(
    ...
    return [
      ...
      ``,
      `*Trend (column, per-symbol only): computed FROM CLOSING PRICES OF CLOSED SIGNALS of that symbol (one (closeTimestamp, currentPrice) point per closed trade — no candles, no order book, no tick stream). Bivariate classification: "sideways" when R² < 0.30; otherwise "neutral" when |slope| < 0.25 × medianStepSize; otherwise "bullish"/"bearish" by sign of slope. Two-axis gate, never a single magic constant on slope magnitude. Not aggregated portfolio-wide (price series across symbols are not comparable).*`,
      `*Trend %/d (column = trendStrength, per-symbol only): computed FROM CLOSING PRICES OF CLOSED SIGNALS of that symbol. Ordinary-least-squares slope of \`log(close) ~ days\` over the symbol's stored closed-trade series, in **%/day** (small-slope approx). NOT ATR-normalised, NOT ADX, NOT a rolling-window indicator — static fit at report time.*`,
      `*Trend R² (column = trendConfidence, per-symbol only): computed FROM CLOSING PRICES OF CLOSED SIGNALS of that symbol. Coefficient of determination of that same log-price regression. On **log(close)**, NOT on absolute close. Range [0, 1]; high R² = clean trend; low R² = noisy regardless of slope sign.*`,
      `*Buyer Pres / Seller Pres (columns, per-symbol only): computed FROM CLOSING PRICES OF CLOSED SIGNALS of that symbol. FREQUENCY of up vs down moves: up-moves = count of close[i] > close[i−1]; flats excluded. Σpres = 1. NOT order-flow aggressor volume — labels are on the SIGN of the close-to-close return; the report has no orderbook access.*`,
      `*Buyer Str / Seller Str (columns, per-symbol only): computed FROM CLOSING PRICES OF CLOSED SIGNALS of that symbol. MAGNITUDE share — Σ|close[i] − close[i−1]|/close[i−1] over up-moves (resp. down-moves), divided by the same sum over ALL decisive moves. Σstr = 1. Pressure (count) and Strength (Σ|Δ|) use the SAME return series; divergence between them reveals regime asymmetry.*`,
      `*Pres Imb (column = pressureImbalance, per-symbol only): computed FROM CLOSING PRICES OF CLOSED SIGNALS of that symbol. DIFFERENCE, not ratio: buyerStrength − sellerStrength ∈ [−1, +1]. Sign = direction of magnitude bias.*`,
      `*Median Step (column = medianStepSize, per-symbol only): computed FROM CLOSING PRICES OF CLOSED SIGNALS of that symbol. Median over i ∈ [1..N−1] of |close[i] − close[i−1]| / close[i−1] — the PERCENT step between consecutive trade closes (NOT ticks, NOT bars; the report has no candle access). Normalised by price, so directly comparable across symbols at any price level. NOT classical candle-based volatility; measures the step distribution at the rate trades close.*`,
