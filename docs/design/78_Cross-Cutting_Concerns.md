...
graph TB
    ...
    subgraph "Logging Infrastructure"
        ...
        ILogger["ILogger Interface<br/>log/debug/info/warn/error"]
        MethodNames["METHOD_NAME constants<br/>String identifiers for log topics"]
    end
    
    subgraph "Error Infrastructure"
        ErrorEmitter["errorEmitter<br/>(Subject<Error>)"]
        ExitEmitter["exitEmitter<br/>(Subject<Error>)"]
        ListenError["listenError()<br/>Recoverable errors"]
        ListenExit["listenExit()<br/>Fatal errors"]
        ValidationError["Validation Error Emission<br/>StrategyValidationService throws"]
    end
    
    subgraph "Persistence Infrastructure"
        PersistBase["PersistBase<EntityName, Entity><br/>Abstract class"]
        ...
        ScheduleAdapter["PersistScheduleAdapter<br/>Scheduled signal persistence"]
        PartialAdapter["PersistPartialAdapter<br/>Partial profit/loss tracking"]
    ...
    PersistBase --> ScheduleAdapter
    PersistBase --> PartialAdapter
    SignalAdapter --> AtomicWrites
    RiskAdapter --> AtomicWrites
    ScheduleAdapter --> AtomicWrites
    PartialAdapter --> AtomicWrites
    
    MethodNames -.-> "All Services"
...
graph TB
    subgraph "Error Sources"
        ValidationFail["StrategyValidationService<br/>throws Error"]
        RiskCheckFail["ClientRisk.checkSignal()<br/>throws Error"]
        PersistFail["Persistence Failure<br/>writeValue() throws"]
        ExchangeFail["Exchange API Failure<br/>getCandles() throws"]
        BackgroundFatal["Background Task Failure<br/>Unhandled exceptions"]
    end
    
    subgraph "Error Processing"
        TryCatch["try-catch Wrappers<br/>In Logic Services"]
        ErrorEmitter["errorEmitter.next()<br/>Subject<Error>"]
        ExitEmitter["exitEmitter.next()<br/>Subject<Error>"]
        QueuedProcessing["Queued Processing<br/>queued() wrapper"]
    end
    
    subgraph "Error Handling"
        ListenError["listenError(callback)<br/>Recoverable errors"]
        ListenExit["listenExit(callback)<br/>Fatal errors"]
        BackgroundMode["Background Mode<br/>Continues execution"]
        ForegroundMode["Foreground Mode<br/>Throws to caller"]
    end
    ...
    BackgroundFatal --> ExitEmitter
    
    TryCatch --> ErrorEmitter
    ErrorEmitter --> QueuedProcessing
    ExitEmitter --> QueuedProcessing
    QueuedProcessing --> ListenError
    QueuedProcessing --> ListenExit
    ListenError --> BackgroundMode
    ListenExit --> BackgroundMode
    TryCatch -.->|"foreground mode"| ForegroundMode
```
